...
Paso 1) Seleccionar el mate
    ...
    Paso 11) Calentar agua en la pava
        ...
        Momento decisivo: 
                    ...
                    Paso 12) Si el mate no tiene agua, llenarlo con agua de la pava 

                    Paso 13) Si el mate tiene agua:
...
El celular tiene un interfas de componenetes IU, te permiten compilar la interfaz gráfica de usuario para tu app. Tambien cuenta con un softward, sistema operativo móvil. (Symbian, Microsoft, Palm OS, Linux, Android, iPhone)
...
_Prompt_: Python 3.10.2 (tags/v3.10.2:a58ebcc, Jan 17 2022, 14:12:15) [MSC v.1929 64 bit (AMD64)] on win32
Type "help", "copyright", "credits" or "license" for more information.

Podemos conlcuir que el _Promt_ de ambas es el mismo.

La terminal de _Visual Code_ entiende lenguaje Python.
...
El resultado es "15",  `*` simbolisa la cuenta de multiplicacion.
...
El resultado es "2",  `/` simbolisa la cuenta de divicion.
...
En las primeras dos invocaciones, el resultado es un número, por lo tanto es un dato de tipo int. 
El último se refiere a una variable de igualdad.
...
El resultado nos devuelve el tipo de dato llamado string ya que se trata de una data representado en forma de texto.
...
6, ya que el espacio entre Marie y Curie se cuenta.
...
El tipo de dato del resultado es el mismo que el anterior, un string. En caso de hacer saludo[0:], nos devuelve el mismo saludo: "Hola mundo".
...
saludo = "Hola mundo" al invocarlo no devuelve nada ya que se está creando una nueva variable pero... 
...
La siguiente invocacion: saludo.replace("o", "a"), remplaza en el saludo la letra "o" por la "a" en todo el string.

> Para pensar 🤔: ¿Se podrán combinar los métodos? Probá hacer `saludo.upper().lower()` ¿Qué dá? ¿Por qué?
>

Si se puede hacer saludo.upper().lower() y te devuelve la última función que agregaste, es decir "hola mundo".
...
Al probar saludo.replace("mundo", "terricolas") y luego saludo me devuelve el mismo valor que el original ya que la variable no cambia. La respuesta es "Hola mundo". 

Para que cambia debería de hacerse:

 saludo = saludo("mundo", "terricolas")

>📚 Para investigar: ¿Qué es la inmutabilidad en Python?

Es una peculiaridad de Python. Es una manera de tratar los datos como mutables o inmutables.

Si el valor puede cambiar, el objeto se llama mutable.
Si el valor no puede cambiar, el objeto se llama inmutable.
...
Haciendo len(lista) te devuelve los datos de la longitud de la lista.
...
`lista.index('25')`, el resultado que obtenemos es error. Ya que ese numero no pertenece a la lista.

La funcion _index()_ sirve para saber en que posicion se encuentra un elemento dentro de la lista previamente invocada.
...
Para obtener los valores se invoca el nombre del diccionario previamente etiquetado.
...
>
    ...
    Ejemplo: termos_por_ronda(40) = 1.2 
    Se consumió 1 termo completo
...
>
    ...
    calcular_cantidad_de_agua(40) = 'vas a necesitar más de un térmo'

    calcular_cantidad_de_agua(25) = 750
...
> Para pensar 🤔: ¿Qué nos imprime el procedimiento de anterior? ¿Qué significa _i_?
    ...
    el proceso anterior imprime pedido de la lista_comensales.
    _i_ se puede referir a un contador, o un elemento en la lista. 
...
>
    ...
    dict1 = {"gusto1":0, "gusto2":0, "gusto3":0}
    dict1["gusto1"] += 1}
